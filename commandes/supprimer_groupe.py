    ...
    async def supprimer_groupe(interaction: discord.Interaction, groupe:str):
        ...
        if interaction.user.id == 334695006663344151:
            ...
            if bdd._delete('groupe', ['id'], (groupe,)):
                ...
                bdd.disconnect()
                await interaction.followup.send(embed=embed_service.obtenir_erreur(f"Impossible de supprimer ce groupe car il n'existe pas.", bot.user.display_avatar.url))
